...
def inspect_plate(image_path):
    ...
    if len(circles_sorted) < 2:
        print("Error: Not enough circles detected to perform distance calculation.")
        return False
    ...
    # --- New and improved calculation of plate dimensions based on hole positions ---
    ...
    measured_plate_width_cm = hole_positions_cm[1][0] + KNOWN_DIMENSIONS["dist_edge_to_hole_x_cm"]
    measured_plate_height_cm = hole_positions_cm[2][1] + KNOWN_DIMENSIONS["dist_edge_to_hole_y_cm"]
    ...
    if len(circles_orig_sorted) < 4:
        print("Error: Not enough circles detected to perform all annotations.")
    ...
    for i, c_orig in enumerate(circles_orig_sorted):
        ...
        if i < len(measured_hole_diameters_cm):
            ...
            cv2.putText(canvas, f"R: {radius_cm:.2f} cm", text_pos, font, 0.5, (0, 0, 0), 2)
    ...
    if len(circles_orig_sorted) >= 4:
        ...
        cv2.line(canvas, (circles_orig_sorted[0][0], circles_orig_sorted[0][1]), (circles_orig_sorted[1][0], circles_orig_sorted[1][1]), (0, 0, 0), 1)
        cv2.putText(canvas, f"{dist_between_holes_x_top_cm_measured:.2f} cm", (int((circles_orig_sorted[0][0] + circles_orig_sorted[1][0]) / 2), circles_orig_sorted[0][1] - 25), font, 0.5, (0, 0, 0), 2)
        
        # Bottom Horizontal
        dist_between_holes_x_bottom_cm_measured = abs(circles_sorted[3][0] - circles_sorted[2][0]) * pixel_to_cm_ratio
        cv2.line(canvas, (circles_orig_sorted[2][0], circles_orig_sorted[2][1]), (circles_orig_sorted[3][0], circles_orig_sorted[3][1]), (0, 0, 0), 1)
        cv2.putText(canvas, f"{dist_between_holes_x_bottom_cm_measured:.2f} cm", (int((circles_orig_sorted[2][0] + circles_orig_sorted[3][0]) / 2), circles_orig_sorted[2][1] + 25), font, 0.5, (0, 0, 0), 2)

        # Left Vertical
        dist_between_holes_y_left_cm_measured = abs(circles_sorted[2][1] - circles_sorted[0][1]) * pixel_to_cm_ratio
        cv2.line(canvas, (circles_orig_sorted[0][0], circles_orig_sorted[0][1]), (circles_orig_sorted[2][0], circles_orig_sorted[2][1]), (0, 0, 0), 1)
        cv2.putText(canvas, f"{dist_between_holes_y_left_cm_measured:.2f} cm", (circles_orig_sorted[0][0] - 80, int((circles_orig_sorted[0][1] + circles_orig_sorted[2][1]) / 2)), font, 0.5, (0, 0, 0), 2)

        # Right Vertical
        dist_between_holes_y_right_cm_measured = abs(circles_sorted[3][1] - circles_sorted[1][1]) * pixel_to_cm_ratio
        cv2.line(canvas, (circles_orig_sorted[1][0], circles_orig_sorted[1][1]), (circles_orig_sorted[3][0], circles_orig_sorted[3][1]), (0, 0, 0), 1)
        cv2.putText(canvas, f"{dist_between_holes_y_right_cm_measured:.2f} cm", (circles_orig_sorted[1][0] + 20, int((circles_orig_sorted[1][1] + circles_orig_sorted[3][1]) / 2)), font, 0.5, (0, 0, 0), 2)
