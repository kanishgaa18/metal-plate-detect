...
# Dimensions are already in pixels, no conversion needed
length_pixels = h
breadth_pixels = w

# Draw bounding box and text for the main object
cv2.rectangle(output, (x, y), (x + w, y + h), (0, 255, 0), 2)
cv2.putText(output, f"Length: {length_pixels} px", (x, y - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
cv2.putText(output, f"Breadth: {breadth_pixels} px", (x, y - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
...
if hole is not None:
    ...
    for i in hole[0, :]:
        # Calculate the diameter in pixels (radius * 2)
        ...
        # Display the diameter in pixels
        cv2.putText(
            output,
